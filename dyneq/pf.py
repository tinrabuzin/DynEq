...
class StudyCase(SubscribablePFObject):
    ...
    def __init__(self, pfobject, *args, **kwargs):
        ...
        nets = set()
        for ref in summary.GetChildren(1):
            if ref.obj_id is not None:
                nets.add(ref.obj_id.loc_name)                
        self.networks = nets
...
class ElmRes(BasePFObject):
    ...
    def variables(self):
        ...
        for j in range(self.n_cols):
            ...
            if self.naming == 'full_name':
                vars[f'{elm.loc_name}.{elm.GetFullName()}'].append(var)
            elif self.naming == 'reduced':
    ...
    def variables(self, outputs):
        ...
        for elm_name, var_names in outputs.items():
            for element in pfhandle.GetCalcRelevantObjects(elm_name.split('\\')[-1]):
                ...
                if self.naming == 'full_name' and (elm_name in full_name):
                    name = full_name
                elif  self.naming == 'reduced' and (elm_name in full_name.split('\\')[-1]):
                    name = full_name.split('\\')[-1]
                elif self.naming == 'user_defined' and ( (elm_name in full_name) or (elm_name in full_name.split('\\')[-1])):
                    name = elm_name
                else:
                    continue
                for variable in var_names:
                    self.AddVariable(element, variable) # CHANGE TO THE NEWER VERSION
                    self.head.append(name+'\\'+variable)
    ...
    def outputs(self):
        ...
        if self.naming == 'user_defined':
            var_names = self.head
            if len(self.head) != self.n_cols:
                raise Exception(f'The number of columns is not the same as the number of variables specified when setting outputs!')
        else:
            var_names = []
            for j in range(self.n_cols):
                elm = self.GetObject(j)
                var = self.GetVariable(j)
                if self.naming == 'full_name':
                    var_names.append(f'{elm.GetFullName()}\\{var}')
                elif self.naming =='reduced':
